...
while True:
    for searchterm in searchterms:
        for tweet in tweepy.Cursor(api.search, q=searchterm).items(10):
            if tweet in tweets:
                ...
                tweets.append(tweet.text)  # Adding tweet to db
                if tweet.author.id not in names:  # Checking if same user tweeted more than once
    ...
    for i in names:
        try:
            api.create_friendship(i)  # Following user who tweeted with #PyConIndia #PyConIndia2018
        except:
            pass
    time.sleep(15)
